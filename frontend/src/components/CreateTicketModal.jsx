...
const CreateTicketModal = ({ onClose, complexId }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.status === 201) {
        alert('✓ Tichet creat cu succes! Administratorii complexului au fost notificați.');
        onClose();
      }
    } catch (err) {
      const message = err.response?.data?.message || 'Eroare la crearea tichetu';
      setError(message);
      console.error('Eroare la crearea tichetu:', err);
    } finally {
      setLoading(false);
    }
  };
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ marginBottom: '16px' }}>
            ...
            >
              ...
              <option value="HVAC">Climatizare/Încălzire 🌡️</option>
              <option value="CLEANING">Curățenie 🧹</option>
              <option value="STRUCTURAL">Structură/Zidărie 🏗️</option>
              <option value="FURNITURE">Mobilier/Echipamente 🪑</option>
          ...
          <div style={{ marginBottom: '16px' }}>
            ...
            >
              ...
              <option value="MEDIUM">Medie - In cateva zile</option>
              <option value="HIGH">Ridicata - Urgent</option>
              <option value="URGENT">Foarte Urgenta - Acum!</option>
            </select>
          </div>
